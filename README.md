...
- **名字库**：约 210 个编辑手挑的名字，每条都带一句话释义，热门名字带「热门」标签
- **卡片操作**：单个名字支持「收藏」「复制」与「换一个」，筛选太严格时自动放宽
- **收藏夹**：一键收藏心动名字，存在浏览器本地（localStorage），随时回看、一键复制
- **今日幸运名**：按日期随机推荐一个名字，每天不重样
- **命中计数**：筛选时实时显示当前条件下的候选名字数量
- **起名避坑指南**：谐音梗、生僻字等四条血泪经验
